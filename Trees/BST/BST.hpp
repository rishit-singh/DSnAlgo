...
protected:
	...
	NodeTuple<T> SeekEnd(std::shared_ptr<Node<T>>, std::shared_ptr<Node<T>>, Direction = Direction::Left);
...
{
	return this->SeekNode(value, nullptr, this->Root);
} 
...
template<typename T>
NodeTuple<T> BinarySearchTree<T>::SeekEnd(std::shared_ptr<Node<T>> parent, std::shared_ptr<Node<T>> node, Direction direction)
{
	if ((direction == Direction::Left && !node->Left) || (direction == Direction::Right && !node->Right))
		std::make_tuple(parent, node, direction);

	else if (direction == Direction::Left)
		return SeekEnd(node, node->Left, direction);

	return SeekEnd(node, node->Right, direction); // the only remaining condition is right
}
...
T BinarySearchTree<T>::Delete(T value)
{
	...
	{
		NodeTuple<T> end = this->SeekEnd(node, node->Right, Direction::Right);
		
		auto endParent = std::get<0>(end);
		auto endNode = std::get<1>(end);

		Direction endDirection = std::get<2>(end);

		if (endNode == node->Right)
			node->Right = nullptr;

		if (endDirection == Direction::Left)
			endParent->Left = node->Right;
		
		else if (endDirection == Direction::Right)
			endParent->Right = node->Right;
